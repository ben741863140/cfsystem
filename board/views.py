...
def board_rating(request, id = -1):
    if request.user.is_authenticated:
        ...
        users = []
        if id == -1:
            id = Board.objects.order_by('id').first().id
        time = Board.objects.filter(id=id).get().name
        for rc in BoardItem.objects.filter(board=Board.objects.filter(id=id).get()):
            user = User()
            user.change = rc.max_rating - rc.oldRating
            if user.change <= 0:
                continue
        ...
        return render(request, 'board/board_upgrade.html', {'users': users, 'time': time, 'boards': Board.objects.all()})
    else:
